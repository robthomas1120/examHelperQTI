...
class QTIToPDFUI {
  ...
  initElements() {
    ...
    this.elements = {
      ...
      downloadAnswerKeyBtn: document.getElementById(
        "qti-download-answer-key-btn"
      ),
  ...
  attachEventListeners() {
    ...
    if (this.elements.dropArea) {
      this.elements.dropArea.addEventListener(
        "dragover",
      ...
      this.elements.dropArea.addEventListener(
        "click",
        this.handleDropAreaClick.bind(this)
      );
    }
    ...
    if (this.elements.removeFileBtn) {
      this.elements.removeFileBtn.addEventListener(
        "click",
        this.handleRemoveFile.bind(this)
      );
    }

    // Convert button
    if (this.elements.convertBtn) {
      this.elements.convertBtn.addEventListener(
        "click",
    ...
    if (this.elements.downloadAnswerKeyBtn) {
      this.elements.downloadAnswerKeyBtn.addEventListener(
        "click",
        this.handleDownloadAnswerKeyClick.bind(this)
      );
    }
  ...
  async previewQTIContent(file) {
    try {
      ...
      for (const [path, zipEntry] of Object.entries(zip.files)) {
        ...
        if (
          lowercasePath.includes("questions.xml") ||
    ...
    try {
      ...
      for (let i = 0; i < previewCount; i++) {
        ...
        const typeNode = item.querySelector(
          "fieldlabel:contains('question_type'), fieldlabel"
        );
      ...
      if (itemNodes.length > maxPreviewQuestions) {
        previewHtml += `
                    <div class="preview-more">
                        ... and ${
                          itemNodes.length - maxPreviewQuestions
                        } more questions
                    </div>
                `;
      }
  ...
  async handleConvertClick(e) {
    ...
    try {
      ...
      if (this.elements.includeAnswers.checked) {
        ...
        const answerKeyPdfBlob = await answerKeyConverter.convertToPDF(
          this.currentFile
        );
  ...
  handleDownloadAnswerKeyClick(e) {
    if (!this.answerKeyPdfBlob) {
      this.showError(
        "No answer key PDF available for download. Please convert first."
      );
      return;
    }
  ...
  updateButtonState() {
    if (this.elements.convertBtn) {
      this.elements.convertBtn.disabled =
        !this.currentFile || !this.elements.titleInput.value.trim();
    }
  }
  ...
  updateResults(success) {
    if (success) {
      ...
      // Update summary
      const paperSize =
        this.elements.paperSize.options[this.elements.paperSize.selectedIndex]
          .text;
      const college =
        this.elements.collegeSelect.options[
          this.elements.collegeSelect.selectedIndex
        ].text;
      ...
      this.elements.conversionSummary.innerHTML = `
                ...
                <p><strong>Answer Key Generated:</strong> ${
                  this.elements.includeAnswers.checked ? "Yes" : "No"
                }</p>
            `;
